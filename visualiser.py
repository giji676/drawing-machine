...
# load inputs
with open("tracemaker_input.txt") as f:
    f = f.readlines()

tsp_file = f[0].strip("\n")
cyc_file = f[1].strip("\n")
output_image = f[2].strip("\n")
output_tracepath = f[3].strip("\n")
...
with open(cyc_file) as f:
    ...
    for line in f:
        ...
        arr = line.split()
        arr = [int(x) for x in arr]
        cyc_file_arr.append(arr)

for segment_ind in range(segment_nbr):
    point_ind0, point_ind1, dist_int = cyc_file_arr[segment_ind]
    if segment_ind == 0:
...
with open(output_tracepath, 'w') as trace_path_file:
    for row in trace_path:
        trace_path_file.write(' '.join([str(a) for a in row]) + '\n')
